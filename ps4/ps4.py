...
def build_coder(shift):
    ...
    it = range(len(bigLetters))
    for i in it:
        key[bigLetters[i]] = bigLetters[ (i+shift) % 27]
        key[smallLetters[i]] = smallLetters[ (i+shift) % 27]
    return key    
...
def find_word(wordlist, text, start = 1):
    shiftLoc = 0
    arr = []
    for i in range(start,27):
        unText = apply_coder(text, build_decoder(i))
        if is_word(wordlist, unText):
            arr.append(i)
    return arr   
